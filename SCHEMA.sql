...
DO $pol$ BEGIN
  ...
  CREATE POLICY "health_decl_access" ON health_declarations FOR ALL USING (
    EXISTS (SELECT 1 FROM bookings b WHERE b.id = booking_id AND (b.instructor_id = auth.uid() OR auth.uid() = (SELECT host_id FROM venues WHERE id = b.venue_id)))
  );
